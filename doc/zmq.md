...
The MindBlockchain Core daemon can be configured to act as a trusted "border
router", implementing the mindblockchain wire protocol and relay, making
...
The ZeroMQ feature in MindBlockchain Core requires the ZeroMQ API >= 4.0.0
...
during the *configure* step of building mindblockchaind:
...
For instance:
    ...
    $ mindblockchaind -zmqpubhashtx=tcp://127.0.0.1:246031 \
               -zmqpubhashtx=tcp://192.168.1.2:246031 \
               -zmqpubrawtx=ipc:///tmp/mindblockchaind.tx.raw \
...
These options can also be provided in mindblockchain.conf.
...
From the perspective of mindblockchaind, the ZeroMQ socket is write-only; PUB
sockets don't even have a read function. Thus, there is no state
introduced into mindblockchaind directly. Furthermore, no information is
...
using. MindBlockchaind appends an up-counting sequence number to each
