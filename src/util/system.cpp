...
const char * const MINDBLOCKCHAIN_CONF_FILENAME = "mindblockchain.conf";
const char * const MINDBLOCKCHAIN_SETTINGS_FILENAME = "settings.json";
 ...
 * https://github.com/mindblockchain/mindblockchain/pull/16097#issuecomment-514627823
 */
...
bool ArgsManager::ParseParameters(int argc, const char* const argv[], std::string& error)
{
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (key == "-") break; //mindblockchain-tx using stdin
        std::string val;
...
bool ArgsManager::GetSettingsPath(fs::path* filepath, bool temp) const
{
    ...
    if (filepath) {
        std::string settings = GetArg("-settings", MINDBLOCKCHAIN_SETTINGS_FILENAME);
...
#else
    const char* pszModule = "mindblockchain";
#endif
    if (pex)
        return strprintf(
...
{
    // Windows: C:\Users\Username\AppData\Roaming\MindBlockchain
    // macOS: ~/Library/Application Support/MindBlockchain
    // Unix-like: ~/.mindblockchain
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "MindBlockchain";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // macOS
    return pathRet / "Library/Application Support/MindBlockchain";
#else
    // Unix-like
    return pathRet / ".mindblockchain";
#endif
#endif
}
...
bool ArgsManager::ReadConfigFiles(std::string& error, bool ignore_invalid_keys)
{
    ...
    const std::string confPath = GetArg("-conf", MINDBLOCKCHAIN_CONF_FILENAME);
