...
uint256 SendCoins(CWallet& wallet, SendCoinsDialog& sendCoinsDialog, const CTxDestination& address, CAmount amount, bool rbf)
{
    ...
    entry->findChild<MindBlockchainAmountField*>("payAmount")->setValue(amount);
...
//
//     QT_QPA_PLATFORM=xcb     src/qt/test/test_mindblockchain-qt  # Linux
//     QT_QPA_PLATFORM=windows src/qt/test/test_mindblockchain-qt  # Windows
//     QT_QPA_PLATFORM=cocoa   src/qt/test/test_mindblockchain-qt  # macOS
void TestGUI(interfaces::Node& node)
{
...

    {
        ...
        QString balanceComparison = MindBlockchainUnits::formatWithUnit(unit, balance, false, MindBlockchainUnits::SeparatorStyle::ALWAYS);
    ...
    // Amount input
    MindBlockchainAmountField* amountInput = receiveCoinsDialog.findChild<MindBlockchainAmountField*>("reqAmount");
    ...
    for (QWidget* widget : QApplication::topLevelWidgets()) {
        if (widget->inherits("ReceiveRequestDialog")) {
            ...
            QCOMPARE(uri.count("mindblockchain:"), 2);
...
void WalletTests::walletTests()
{
    ...
    if (QApplication::platformName() == "minimal") {
        ...
        QWARN("Skipping WalletTests on mac build with 'minimal' platform set due to Qt bugs. To run AppTests, invoke "
              "with 'QT_QPA_PLATFORM=cocoa test_mindblockchain-qt' on mac, or else use a linux or windows build.");
        return;
    }
#endif
    TestGUI(m_node);
