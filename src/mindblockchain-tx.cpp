...
static void SetupMindBlockchainTxArgs(ArgsManager &argsman)
{
...
{
    SetupMindBlockchainTxArgs(gArgs);
    std::string error;
    ...
    if (argc < 2 || HelpRequested(gArgs)) {
        ...
        std::string strUsage = PACKAGE_NAME " mindblockchain-tx utility version " + FormatFullVersion() + "\n\n" +
            "Usage:  mindblockchain-tx [options] <hex-tx> [commands]  Update hex-encoded mindblockchain transaction\n" +
            "or:     mindblockchain-tx [options] -create [commands]   Create hex-encoded mindblockchain transaction\n" +
            "\n";
...
static int CommandLineRawTx(int argc, char* argv[])
{
    ...
    try {
        ...
        if (!fCreateBlank) {
            ...
            // param: hex-encoded mindblockchain transaction
