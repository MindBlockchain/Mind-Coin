...
static bool WalletAppInit(int argc, char* argv[])
{
    ...
    if (argc < 2 || HelpRequested(gArgs)) {
        std::string usage = strprintf("%s mindblockchain-wallet version", PACKAGE_NAME) + " " + FormatFullVersion() + "\n\n" +
                                      "mindblockchain-wallet is an offline tool for creating and interacting with " PACKAGE_NAME " wallet files.\n" +
                                      "By default mindblockchain-wallet will act on wallets in the default mainnet wallet directory in the datadir.\n" +
...
int main(int argc, char* argv[])
{
    ...
    if (method.empty()) {
        tfm::format(std::cerr, "No method provided. Run `mindblockchain-wallet -help` for valid methods.\n");
        return EXIT_FAILURE;
    }
