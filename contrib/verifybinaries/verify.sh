...
###   mindblockchaincore.org and mindblockchain.org and compares them.
...
WORKINGDIR="/tmp/mindblockchain_verify_binaries"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test"
HOST1="https://mindblockchaincore.org"
HOST2="https://mindblockchain.org"
BASEDIR="/bin/"
VERSIONPREFIX="mindblockchain-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'mindblockchain-',
...
if ! WGETOUT=$(wget -N -O "$SIGNATUREFILENAME.2" "$HOST2$BASEDIR$SIGNATUREFILENAME" 2>&1); then
   echo "mindblockchain.org failed to provide signature file, but mindblockchaincore.org did?"
   echo "wget output:"
   # shellcheck disable=SC2001
   echo "$WGETOUT"|sed 's/^/\t/g'
...
if [ "$SIGFILEDIFFS" != "" ]; then
   echo "mindblockchain.org and mindblockchaincore.org signature files were not equal?"
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the MindBlockchain Core binary release signing key installed?"
   fi
